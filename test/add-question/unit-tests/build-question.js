...
describe('buildQuestion(body)', () => {
  it('Should return an object with the attribute obj.question', () => {
    const req = {body: {question: 'Test', a: 'a1', b: 'b2', c: 'c3', d: 'd4', correct: 'b', categories: ['a']}};
  ...
  it('Should return an object with an array obj.categories', () => {
    const req = {body: {question: 'Test', a: 'a1', b: 'b2', c: 'c3', d: 'd4', correct: 'b', categories: ['a']}};
    const q = buildQuestion(req.body);
    expect(Array.isArray(q.categories)).to.eql(true);
    expect(q.categories.length).to.eql(1);
  });
});
